...
// WordPress globals - keeping the working approach
declare const wp: {
    hooks: any;
    blocks: any;
    compose: any;
...
export class AnimationValidator {
    ...
    static validateConfig(config: AnimationConfig): { isValid: boolean; errors: string[] } {
        ...
        // Validate properties
        if (config.properties.x && typeof config.properties.x === 'string') {
            if (!this.validateCSSUnit(config.properties.x)) {
                errors.push('X movement must be a valid CSS unit');
            }
        }

        if (config.properties.y && typeof config.properties.y === 'string') {
